...
> [!NOTE]
> `ChangeFeed.WebhookReceiverBase` must be a URL Edge can actually reach. In local development this means a public tunnel (the Starter Kit's sample config uses a dev tunnel URL); in production it's your site's public base URL.
...
If your application caches its own data derived from Sitecore (the Starter Kit, for example, caches the main and footer navigation in `SitecoreData`), subscribe to the evictor so you rebuild when content is republished:
