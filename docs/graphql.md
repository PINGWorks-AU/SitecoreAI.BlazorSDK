...
Most of the time you don't need to think about GraphQL at all - the SDK fetches the layout for a page, binds it to your component models, and you work with strongly-typed fields. But sometimes you need data that isn't part of the current page's layout: a navigation tree, a breadcrumb trail, a list of related articles, a global settings item. For those cases the SDK gives you a typed GraphQL client over Sitecore Experience Edge, and the Starter Kit includes a code-generation workflow that turns your Sitecore schema into C# query classes.

## The two pieces

| Piece | What it is |
| - | - |
| `Website.GraphQL` project | A class library of generated C# types - one per Sitecore template and Edge type - that lets you build queries in a fluent, compile-checked way. |
| `Generate-GraphQL.ps1` | The script that regenerates that project from your Sitecore Edge endpoint. |
...
Then run the script from the solution root:

```cmd
.\Generate-GraphQL.ps1
```

The script invokes the tool against your Sitecore Edge GraphQL endpoint and writes the generated classes into `apps/Website.GraphQL`. Its key arguments are:

- `-u` - the Edge GraphQL endpoint. The Starter Kit points at the environment's Preview endpoint (`https://[environment].sitecorecloud.io/sitecore/api/graph/edge`). The live delivery endpoint (`https://edge.sitecorecloud.io/api/graphql`) is noted in the script's comments.
- `-h` - request headers. The script supplies an `sc_apikey` header whose value you are prompted for when you run it, so the key never lands in source control.
- `-n` - the namespace for the generated classes (`Website.GraphQL`).
- `-o` - the output folder.
- `--force` - overwrite existing generated files.
- `--public` - generate public types.

Before you run it, update the endpoint URL in the script to match your own environment.

> [!NOTE]
> The generated files are committed to the repo so the solution builds without anyone needing the tool. Regenerate (and commit the result) whenever your Sitecore templates change in a way you want to query - new templates, new fields, renamed fields.
...
Here is a trimmed version of the Starter Kit's breadcrumb query (`Repositories/SitecoreData.cs`):
...
1. **Make sure the types exist.** If you're querying templates or fields that already existed when the client was last generated, the C# types are already there. If you've added templates or fields in Sitecore, update the endpoint URL in `Generate-GraphQL.ps1` if needed and rerun it to regenerate the client.
2. **Write a method** (a repository is the natural home - see `SitecoreData.cs`) that injects `ISitecoreGraphQL`, builds the `Query` with the fluent API, executes it and maps the result into a view model your components can consume.
