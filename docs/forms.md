...
`FormReceiver.ReceiveData` (`Apis/Webhooks/FormReceiver.cs`) reads a small set of headers from the request and dispatches the body to the matching processors:
...
Processors are registered in `AddWebsite()` (`Ioc/IServicesCollectionExtensions.cs`):

```csharp
services
	.AddSingleton<IFormProcessor, SaveDataFormProcessor>()
	.AddSingleton<IFormProcessor, SendToTeamsProcessor>()
	.AddSingleton<IFormProcessor, SendToEmailProcessor>();
```
...
| `SendToTeamsProcessor` | `Contact Us` | `Teams` | Posts an Adaptive Card to a Microsoft Teams channel. See below. |
| `SendToEmailProcessor` | `Contact Us` | `Email` | Sends an HTML email of the submission via Azure Communication Services. |
...
## Microsoft Teams webhook setup (SendToTeamsProcessor)

`SendToTeamsProcessor` sends form submissions to a Microsoft Teams channel using an incoming webhook. It reads its webhook URL from the `Teams` configuration section.
...
- Confirm the submitting form sends the api key that `SendToTeamsProcessor` expects, and that its `x-formname` matches `Contact Us`

---

## Email setup (SendToEmailProcessor)

`SendToEmailProcessor` sends an HTML summary of the submission using Azure Communication Services. Configure it through the `Email` section:
