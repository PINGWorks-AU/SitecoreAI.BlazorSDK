...
The Starter Kit is designed to provide a worked example with some skeleton code to reduce the effort required to start working with Blazor and Sitecore AI. Create a new GitHub or Azure DevOps repository and copy the contents of this kit into your new repo. You will need **all** the files and folders to ensure Sitecore AI works correctly, even though you will only be customising the project under `/apps/PINGWebsite`.
...
- `/apps/Website` - path to the Blazor project (rename to something meaningful for your site)
- `/apps/Website/Website.csproj` - the Blazor project file name
- `/apps/Website/UI/App.razor` - if you've renamed the assembly, update the component-scoped CSS reference (`Website.Web.styles.css` by default) so the bundle loads correctly
...
This file contains versions of nuget packages used by Sitecore AI Pages. Check for new versions with the command:
...
This file contains the version of the Sitecore AI Blazor SDK CLI tool. Check for new versions with the command:
...
- `PINGWorks.SitecoreAI.BlazorSDK` - the SDK itself (referenced as a project in the Starter Kit; swap to a `PackageReference` in your own repo)
- `BitzArt.Blazor.Cookies.Server` - server-side cookie access used by analytics consent
- `Azure.Communication.Email` / `Azure.Identity` - used by the `SendToEmailProcessor`
...
{
	"SitecoreSettings": {
		/* Sitecore Deploy -> Projects -> Environments -> Sites -> Name */
		"SiteName": "website",
		"DefaultLanguage": "en",
		...
		/* for Pages editor support */
		"EnableEditingMode": true,
		"EnvironmentHostName": "[YOUR_AUTHORING_HOSTNAME].sitecorecloud.io",

		/* The default page template razor, usually Page. Default 'Page'. */
		"DefaultPageTemplate": "Page",

		/* Add the root URL of your authoring host when deployed, e.g. https://authoring.mysite.com */
		"AuthoringHosts": []
	},

	"SyncSettings": {
		/* One of MemoryAndDisk, MemoryOnly, Disabled */
		...
		/* Webhook detects publish events for cache eviction */
		"ChangeFeed": {
...
Right-click your project in the Visual Studio Solution Explorer and select "Manage User Secrets". Add the following secrets:
