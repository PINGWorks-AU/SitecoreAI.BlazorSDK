...
When you first create a Sitecore AI site, your navigable page items typically inherit from the Sitecore template called `Page`.
...
The SDK surfaces the author's choice on the model as `ViewModelBase.RenderingVariant`, which is bound from the rendering's `FieldNames` parameter (as this is how Sitecore sends it):
...
```razor
@using PINGWorks.Web.UI.Components.Banners
...
Some example components in the Starter Kit that follow this pattern include `Banner`, `Content`, `Promo`, `TwoColumns` and `ThreeColumns`.
