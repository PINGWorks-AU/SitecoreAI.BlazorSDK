...
For this resolution to run, your `Program.cs` must call `app.UseRequestLocalization()`. The Starter Kit does this between `UseHttpsRedirection()` and `UseAntiforgery()`:
...
`AddSitecoreBlazorSDK` configures `RequestLocalizationOptions` for you, only the `app.UseRequestLocalisation()` needs to be called explicitly.
